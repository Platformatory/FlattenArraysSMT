...
//    private static final String ARRAY_ELEMENT_DELIMITER_CONFIG = "array_element_delimiter";
//    private static final String ARRAY_ELEMENT_DELIMITER_DEFAULT = ",";
    ...
    public static final ConfigDef CONFIG_DEF = new ConfigDef()
            .define(DELIMITER_CONFIG, ConfigDef.Type.STRING, DELIMITER_DEFAULT, ConfigDef.Importance.MEDIUM,
                    "Delimiter to insert between field names from the input record when generating field names for the "
                            + "output record");
//            .define(ARRAY_ELEMENT_DELIMITER_CONFIG, ConfigDef.Type.STRING, ARRAY_ELEMENT_DELIMITER_DEFAULT,
//                    ConfigDef.Importance.MEDIUM,
//                    "Delimiter to insert between array element values in case the field is of type Array from the input record "
//                            + "when flattening field value for the array type field");
...
//        arrayElementDelimiter = config.getString(ARRAY_ELEMENT_DELIMITER_CONFIG);
    ...
    private void applySchemaless(Map<String, Object> originalRecord, String fieldNamePrefix,
...
//                newRecord.put(fieldName(fieldNamePrefix, entry.getKey()), entry.getValue());
                String arrayfieldName = fieldName(fieldNamePrefix, entry.getKey());
                newRecord.put(arrayfieldName, flattenArray(arrayfieldName, entry.getValue(), PURPOSE));
                break;
            case MAP:
    ...
    private String flattenArray(String fieldName, Object value, String purpose) {
        if (value == null || !(value.getClass().isArray() || value instanceof List<?>)) {
            throw new DataException(fieldName + " is supposed to be of type Array or List for [" + purpose + "], found: "
                    + value == null ? "null" : value.getClass().getName());
        }
        if (value instanceof List<?>) {
            List<?> source = (List<?>) value;
            value = source.toArray(new Object[source.size()]);
        }
        String flattened = ArrayUtils.toString(value).substring(1);
        flattened = flattened.substring(0, flattened.lastIndexOf("}"));
        return flattened;
    }
    ...
    /**
     * Build an updated Struct Schema which flattens all nested fields into a single
     * struct, handling cases where optionality and default values of the flattened
     * fields are affected by the optionality and default values of parent/ancestor
     * schemas (e.g. flattened field is optional because the parent schema was
     * optional, even if the schema itself is marked as required).
     * 
     * @param schema            the schema to translate
     * @param fieldNamePrefix   the prefix to use on field names, i.e. the
     *                          delimiter-joined set of ancestor field names
     * @param newSchema         the flattened schema being built
     * @param optional          true if any ancestor schema is optional
     * @param defaultFromParent the default value, if any, included via the
     *                          parent/ancestor schemas
     */
    private void buildUpdatedSchema(Schema schema, String fieldNamePrefix, SchemaBuilder newSchema, boolean optional,
            Struct defaultFromParent) {
        for (Field field : schema.fields()) {
            ...
            case BYTES:
                ...
                break;
            case ARRAY:
                final SchemaBuilder builder = SchemaBuilder.string().defaultValue("");
                if (optional)
                    builder.optional();
                newSchema.field(fieldName, builder);
//              buildUpdatedSchema(field.schema(), fieldName, SchemaBuilder.string(), fieldIsOptional, (Struct) fieldDefaultValue);
                break;
            case STRUCT:
    ...
    /**
     * Convert the schema for a field of a Struct with a primitive schema to the
     * schema to be used for the flattened version, taking into account that we may
     * need to override optionality and default values in the flattened version to
     * take into account the optionality and default values of parent/ancestor
     * schemas
     * 
     * @param orig              the original schema for the field
     * @param optional          whether the new flattened field should be optional
     * @param defaultFromParent the default value either taken from the existing
     *                          field or provided by the parent
     */
    private Schema convertFieldSchema(Schema orig, boolean optional, Object defaultFromParent) {
        // Note that we don't use the schema translation cache here. It might save us a
        // bit of effort, but we really
    ...
    private void buildWithSchema(Struct record, String fieldNamePrefix, Struct newRecord) {
        ...
        for (Field field : record.schema().fields()) {
            ...
            case BYTES:
                ...
                break;
            case ARRAY:
                newRecord.put(fieldName, flattenArray(fieldName, record.get(field), PURPOSE));
                break;
            case STRUCT:
